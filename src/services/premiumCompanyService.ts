...
export type PremiumApplicationStatus = 'pending' | 'approved' | 'rejected' | 'changes_requested' | 'expired';

export type PremiumBenefit = { limit: number; used: number; remaining: number };
...
async function request<T>(path: string, options: RequestInit = {}, authenticated = false): Promise<T> {
  ...
  if (authenticated && auth.currentUser !== currentUser) throw new Error('계정이 변경되었습니다. 다시 확인해 주세요.');
  ...
  return payload;
    ...
    if (controller.signal.aborted) throw new Error('응답이 지연되고 있습니다. 신청 상태를 새로고침하여 저장 여부를 확인해 주세요.', { cause: error });
    throw error;
  } finally {
...
export async function listPremiumApplications(cursor = ''): Promise<{ applications: PremiumApplication[]; nextCursor: string | null }> {
...
export async function readPremiumPhoto(file: File): Promise<string> {
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 2 * 1024 * 1024 || file.size === 0) {
    throw new Error('JPG, PNG, WEBP 사진을 2MB 이하로 등록해 주세요.');
  }
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onerror = () => reject(new Error('사진을 읽지 못했습니다. 다시 선택해 주세요.'));
    reader.onload = () => typeof reader.result === 'string' ? resolve(reader.result) : reject(new Error('사진을 확인해 주세요.'));
