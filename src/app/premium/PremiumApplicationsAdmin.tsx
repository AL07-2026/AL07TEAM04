...
import {
  listPremiumApplications, reviewPremiumApplication, type PremiumApplication,
} from '@/services/premiumCompanyService';

const labels: Record<PremiumApplication['status'], string> = {
  pending: '검토 중', changes_requested: '보완 요청', rejected: '반려', approved: '노출 중', expired: '노출 종료',
};

function ReviewForm({ application, onUpdated }: {
  ...
  const editable = application.status === 'pending' || application.status === 'approved';
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : '처리하지 못했습니다. 새로고침 후 확인해 주세요.');
  ...
  return (
    <article className="rounded-2xl border border-[#E0D9C8] bg-white p-5">
      <div className="flex flex-wrap justify-between gap-3">
        ...
        <p>{labels[application.status]} · 무료 잔여 {application.benefit?.remaining ?? '-'}회</p>
      </div>
      {application.imageUrl ? <img src={application.imageUrl} alt={application.companyName + ' 신청 사진'} className="mt-4 aspect-video w-full max-w-sm rounded-xl object-cover" /> : <p className="mt-3 text-rose-700">대표 사진 보완이 필요합니다.</p>}
        ...
        <form className="mt-4" onSubmit={(event) => void submit(event)}>
          <fieldset className="grid gap-3" disabled={saving}>
            <label className="grid gap-2 font-bold">
              처리 선택
              <select className="min-h-11 rounded-lg border p-2" value={decision} onChange={(event) => setDecision(event.target.value as typeof decision)}>
                {application.status === 'approved' ? <option value="end">현재 노출 종료</option> : <>
                  <option value="approved">승인 및 즉시 노출 (1회 차감)</option>
            ...
            <p className="text-sm text-slate-600">승인하면 기업 목록과 롤링 배너의 노출 대상에 반영됩니다. 종료해도 사용 횟수는 환급되지 않습니다.</p>
            <button className="min-h-11 rounded-xl bg-[#173F3A] px-4 font-bold text-white disabled:opacity-60" disabled={saving || (decision === 'approved' && !application.imageUrl)} type="submit">{saving ? '처리 중' : '검토 결과 저장'}</button>
          </fieldset>
...
export function PremiumApplicationsAdmin({ canManage }: { canManage: boolean }): React.JSX.Element {
  ...
  useEffect(() => {
    if (!canManage) return;
    let active = true;
    listPremiumApplications(cursor).then((result) => {
      if (!active) return;
      setApplications(result.applications);
      setNextCursor(result.nextCursor);
    }).catch((cause: unknown) => {
      if (active) setError(cause instanceof Error ? cause.message : '신청 목록을 불러오지 못했습니다.');
    }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [canManage, cursor, refresh]);
  if (!canManage) return <p>운영 관리자만 프리미엄 신청을 검토할 수 있습니다.</p>;
  const reload = (next: string) => { setLoading(true); setError(''); setCursor(next); setRefresh((value) => value + 1); };
  return (
    <section className="grid gap-5">
      ...
      {loading ? <p role="status">신청 목록을 불러오는 중입니다.</p> : error ? <p role="alert" className="text-rose-700">{error}</p> : <>
        {applications.length === 0 ? <p>접수된 신청이 없습니다.</p> : applications.map((application) => (
          <ReviewForm key={application.id + ':' + (application.revision || 0)} application={application} onUpdated={(updated) => {
            setApplications((items) => items.map((item) => item.id === updated.id ? updated : item));
            setNotice('검토 결과가 서버에 저장되었습니다.');
          }} />
        ))}
        {nextCursor ? <button type="button" className="min-h-11 rounded-xl border bg-white px-4 font-bold" onClick={() => reload(nextCursor)}>다음 신청 목록</button> : null}
      </>}
    </section>
  );
}
