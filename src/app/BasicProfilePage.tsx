...
import '@/app/BasicProfilePage.css';
...
export function BasicProfilePage() {
  ...
  const updateField =
    (key: keyof ProfileForm) => (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    <main className="profile-page">
      ...
      <section aria-labelledby="profile-title" className="profile-card">
        <header className="profile-header">
          ...
          <div className="brand">
            <span className="brand-mark">
              <Sparkles fill="currentColor" size={15} />
            </span>
            <span>인재 기본정보</span>
          </div>
        ...
        <form className="profile-form" onSubmit={submitProfile}>
          <div aria-label="전체 3단계 중 1단계" className="progress-track">
            <span />
            <i />
            <i />
          </div>
          <div className="intro">
            ...
            <h1 id="profile-title" aria-label="당신의 경험이 새로운 기회를 만나요.">
              당신의 경험이
              <br />
              <strong>새로운 기회</strong>를 만나요.
            </h1>
          ...
          <label className="field">
            <span>어떤 분야에서 일해 오셨나요?</span>
            <input
              onChange={updateField('field')}
              placeholder="예: 서비스 운영 · 브랜드 마케팅"
              value={form.field}
            />
          </label>
          <label className="field">
            <span>얼마나 경험을 쌓으셨나요?</span>
            <input onChange={updateField('period')} placeholder="예: 12년" value={form.period} />
          </label>
          <label className="field">
            <span>가장 자랑하고 싶은 경험은 무엇인가요?</span>
            <textarea
              onChange={updateField('experience')}
              placeholder="프로젝트와 성과를 자유롭게 들려주세요"
              rows={4}
          ...
          <div className="attachment-panel">
            <div className="attachment-copy">
              ...
              <div>
                <b>이력서 또는 포트폴리오</b>
                <small>선택 사항 · PDF, DOCX · 최대 10MB</small>
              </div>
            </div>
            <input
              accept=".pdf,.doc,.docx"
            ...
            {attachment ? (
              <div className="file-selected">
                ...
                <button aria-label="첨부 파일 삭제" onClick={removeAttachment} type="button">
                  <X size={16} />
                </button>
              </div>
            ) : (
          ...
          {message ? (
            <p
              aria-live="polite"
              className={`form-message${message.includes('저장') ? ' success' : ''}`}
            >
              {message}
            </p>
          ) : null}
          ...
          <p className="privacy-note">
            입력하신 정보는 프로젝트 매칭을 위해서만 안전하게 사용돼요.
          </p>
        </form>
      </section>
    </main>
