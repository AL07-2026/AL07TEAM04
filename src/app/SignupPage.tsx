...
export function SignupPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!form.email.trim() || !isValidEmail(form.email.trim())) {
      setMessage(
        '유효하고 실제 존재하는 이메일 형식(예: user@gmail.com, user@naver.com)을 입력해 주세요.',
      );
      return;
    }
    ...
    >
      <div
        className={cn(
          'w-full mx-auto',
          !isMobile &&
            'max-w-xl md:border md:border-[#E0D9C8] md:bg-white md:p-8 md:rounded-2xl md:shadow-md',
        )}
      >
        {step === 'verification' ? (
          <div className="flex flex-col items-center text-center gap-5 py-4">
            ...
            <div className="flex flex-col gap-1.5">
              <h2 className="ui-section-title">이메일 인증을 완료해주세요</h2>
              <p className="ui-caption-text max-w-md mx-auto">
                <span className="font-extrabold text-[#173F3A] bg-[#173F3A]/10 px-2.5 py-1 rounded-md inline-block mb-1.5">
                  {form.email}
                </span>
                <br />위 이메일 주소로 파이어베이스 인증 링크가 발송되었습니다. 메일함을 확인하여
            ...
            {resendStatus ? (
              <p
                aria-live="polite"
                className="text-xs font-extrabold text-[#173F3A] bg-[#173F3A]/10 p-3 rounded-xl w-full"
              >
                {resendStatus}
              </p>
            ) : null}
            ...
            {message ? (
              <p
                aria-live="polite"
                className="text-xs font-extrabold text-rose-500 bg-rose-50 p-3 rounded-xl w-full"
              >
                {message}
              </p>
            ) : null}
          ...
          <form className="flex flex-col gap-3.5" onSubmit={submit}>
            ...
            <div className="flex w-full rounded-full border border-[#E0D9C8] bg-[#FAF7F2] p-1 shadow-2xs mb-1">
              <button
                ...
                className={cn(
                  'ui-segment-button flex flex-1 items-center justify-center gap-1 rounded-full font-extrabold transition-all',
                  selectedRole === 'senior'
            ...
            <div className="flex flex-col gap-1">
              <h2 className="ui-section-title">계정을 만들어 시작하세요</h2>
