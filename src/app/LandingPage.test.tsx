...
describe('LandingPage alignment rail', () => {
  ...
  it('헤더와 모든 랜딩 섹션을 같은 반응형 좌측 기준선에 맞춘다', () => {
    ...
    const headerRail = screen.getByRole('button', { name: '원모어 첫 화면' }).parentElement;
  ...
  it('랜딩 헤더 로고는 공통 28px 높이를 사용한다', () => {
    ...
    expect(screen.getByRole('img', { name: /원모어/i })).toHaveClass('h-7');
  });
  ...
  it('히어로 영상이 별도 중앙 폭을 사용하지 않고 본문 레일 전체를 사용한다', () => {
    ...
    const videoFrame = screen.getByTitle(
      '시니어의 경험과 기업의 과제가 만나는 원모어 소개 영상',
    ).parentElement;
  ...
  it('중복 영문 제목을 제거하고 한국어 제목만으로 정보 위계를 구성한다', () => {
    ...
    expect(screen.getByText('원모어의 서비스 특징')).toHaveClass('font-bold', 'text-[#F06B4F]');
    expect(screen.getByRole('heading', { level: 1 })).toHaveClass('mt-5', 'sm:mt-6');
  });

  it.each(['원모어가 만드는 새로운 연결', '원모어의 서비스 특징', '고용노동부 연계 혜택'])(
