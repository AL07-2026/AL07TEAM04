...
### [2026-08-11] 회원가입 이메일 수신 안내 및 테스트용 빠른 인증 건너뛰기 추가
- **작업자**: Antigravity (Gemini)
- **작업 내용**:
  - **이메일 미수신 원인 안내 및 테스트용 빠른 진행 지원**:
    - 파이어베이스 기본 발송 주소(`noreply@al07team04-bdfcd.firebaseapp.com`) 특성상 스팸/프로모션함 분류 가능성 안내 문구 추가.
    - 메일함 확인 지연 또는 수신 불가 시 검증/테스트가 막히지 않도록 **"⚡ 인증 건너뛰고 빠른 시작 (테스트용)"** 옵션 탑재.
