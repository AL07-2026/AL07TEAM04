...
### [2026-09-14] 기업 프로젝트 모집·공개 상태 연동 및 홈 내 수정
- **작업자/브랜치**: Codex, `feature/job-database`.
- **공용 관리 동작**: 기업 홈과 인재탐색의 등록 프로젝트 팝업이 같은 관리 액션 컴포넌트를 사용. 두 화면 모두 모집·공개 상태를 표시하고 마감/재개, 비공개/재공개, 수정, 삭제를 기존 Firestore 프로젝트 서비스에 연결.
- **인재측 정책**: 공개 프로젝트는 모집 마감 후에도 상세 확인 가능하지만 모든 지원 진입점과 직접 제안 URL에서 저장 차단. 비공개 프로젝트는 인재측 공개 목록에서 제외.
- **홈 내 수정**: 홈에서 페이지 이동 없이 기존 값이 채워진 수정 모달을 열고, 저장 후 동일 관리 팝업으로 복귀. 최근 프로젝트 카드에도 모집·공개 상태 표시.
- **검증**: 관련 TDD 테스트, 타입 검사, ESLint, 프로덕션 빌드 및 단일 워커 전체 69개 파일/711개 테스트 통과. 로그인/API 로컬 경로 HTTP 200 확인.
- **버전 저장**: 기능 소스 커밋 `8f682fa`를 원격 `develop`에 푸시하고 복구 태그 `release/company-project-management-20260914`를 같은 커밋에 생성·푸시.
- **Firebase 배포/확인**: 프로젝트 `al07team04-bdfcd`의 Hosting만 배포. 운영 주소 `https://al07team04-bdfcd.web.app`와 `/api/health` 모두 HTTP 200, 배포 앱 번들 `assets/index-CVSCEk-j.js` 확인. Functions·Firestore 규칙·데이터는 변경하거나 배포하지 않음.
- **복구 기준**: 태그를 별도 브랜치로 체크아웃해 빌드·Hosting 재배포하면 이 버전의 화면으로 복구 가능. `develop` 이력 자체를 되돌릴 때는 강제 푸시 대신 해당 태그 이후 커밋을 `git revert`하여 복구 커밋을 남김.
- **보존**: 사용자 작업물 `output/`, `tmp/`는 커밋과 배포 대상에서 제외하고 그대로 보존.

### [2026-09-14] 기업 프로젝트 관리 팝업 동작 통일 및 추천 인재 상세 추가
- **작업자/브랜치**: Codex, `feature/job-database`.
- **프로젝트 관리**: 기업 홈의 최근 프로젝트와 등록 프로젝트 관리 버튼이 홈에 머문 채 목록·선택 상세 모달을 표시하도록 변경.
- **추천 인재 상세**: 기업 인재탐색에서 인재 카드 선택 시 기본 프로필, 보유 역량, 선택 프로젝트, 적합 점수와 매칭 근거를 표시하고 기존 제안 흐름에 연결.
- **검증**: 타입 검사, ESLint, 프로덕션 빌드 및 당시 전체 69개 파일/708개 테스트 통과.
- **배포**: 커밋·푸시·배포하지 않음.

### [2026-09-14] develop 기준 기업 UI 변경 3건만 재적용
- **작업자/브랜치**: Codex, `feature/job-database`.
- **기준선/범위**: `origin/develop`의 `6d1726e`를 기준으로 기업 메뉴명을 `인재탐색`으로 변경하고 기업 화면에서 프리미엄 기업 영역을 숨김. 기업 홈 프로젝트 관리 모달 추가.
- **검증**: 타입 검사, ESLint, 프로덕션 빌드 및 당시 전체 69개 파일/707개 테스트 통과. 로컬 API health HTTP 200 확인.
- **배포**: 커밋·푸시·배포하지 않음. `output/`, `tmp/` 보존.

### [2026-09-13] 모바일 프리미엄 배너 배치 판정 회귀 수정 및 재배포
- **작업자/브랜치**: Codex. 수정 커밋 `4d2d3e8`을 `develop`과 `leedongwook` 및 두 원격 브랜치에 동일하게 반영.
- **원인**: 홈 화면이 `isCompact`를 화면 폭만으로 판단하지 않고 `max-width: 767px`와 `pointer: coarse`를 함께 사용했다. 따라서 폭이 좁아도 fine pointer로 판정되는 인앱 브라우저에서는 모바일 전용 클래스가 빠져 조작부가 우측 하단, 기업 정보가 그 위에 표시됐다.
- **수정**: 640px 미만 기본 CSS를 모바일 우선으로 전환해 포인터 판정과 무관하게 자동 재생·현재 순번·이전·다음 조작부를 좌측 상단에 고정하고, 기업 정보는 하단 12px 여백의 48px 한 줄 글래스 패널에 고정. 모바일 조작 버튼은 40px, 화살표와 순번 글자는 더 작게 조정해 사진 가림을 줄였으며 일반 PC의 `sm` 배치는 기존 상태로 복원.
- **회귀 방지/검증**: `isCompact=false`에서도 640px 미만 기본 위치가 유지되고 `sm` 이상에서 기존 PC 위치가 복원되는 클래스 계약 테스트를 추가. `npm run validate` 통과: 타입 검사, ESLint, 68개 테스트 파일/704개 테스트, 프로덕션 빌드 성공. 기존 500kB 초과 번들 안내만 유지.
- **배포/확인**: 동일한 62개 `dist` 파일을 `leedongwook` 채널과 정식 Firebase Hosting에 순서대로 재배포. 두 주소 모두 HTTP 200 및 동일한 신규 JS `assets/index-D7iij-l-.js`, CSS `assets/index-DJOO51Eg.css`를 제공함을 확인. 정식 주소의 `assets/FlowPages-DvQ5Zvz3.js`에서도 좌측 상단 조작부와 하단 정보 패널 클래스가 포함되고 기존 우측 하단 모바일 클래스가 제거된 것을 직접 확인.
- **범위/보존**: Hosting만 배포하고 Functions·Firestore 데이터·규칙은 변경하거나 배포하지 않음. 사용자 `Wanted Design System (Community).fig`는 스테이징·수정하지 않음.

### [2026-09-13] 모바일 프리미엄 배너 정보·조작부 분리 및 전체 Hosting 배포
- **작업자/브랜치**: Codex. UI 커밋 `6c0633a`를 `develop`과 `leedongwook` 및 두 원격 브랜치에 동일하게 반영.
- **모바일 배너 개선**: 640px 미만에서 자동 재생·현재 순번·이전·다음 조작부를 사진 좌측 상단의 작은 글래스 영역으로 이동하고, 하단에는 `프리미엄 기업`·기업명·이동 아이콘만 48px 한 줄 글래스 영역으로 유지. 전역 가독성 스타일이 작은 글자를 다시 키우지 않도록 compact 모드 크기를 고정하면서 세 조작 버튼의 44px 터치 영역은 보존.
- **PC 보존**: compact 모드와 일반 모드의 클래스 구성을 분리해 640px 이상에서는 기존 정보 패널의 제목·설명·조작부 배치와 크기를 그대로 유지.
- **검증**: 320·390·448px 비교 화면에서 상단 조작부와 하단 정보가 겹치지 않고 사진 중앙 노출이 늘어난 것을 확인. `npm run validate` 통과: 타입 검사, ESLint, 68개 테스트 파일/703개 테스트, 프로덕션 빌드 성공. 기존 500kB 초과 번들 안내만 유지.
- **배포/확인**: 동일한 62개 `dist` 파일을 `leedongwook` 채널과 정식 Firebase Hosting에 순서대로 배포. 두 주소 모두 HTTP 200 및 동일한 신규 앱 번들 `assets/index-C2HNRjYk.js`를 제공하고 해당 자산도 HTTP 200임을 확인.
- **범위/보존**: Hosting만 배포하고 Functions·Firestore 데이터·규칙은 변경하거나 배포하지 않음. 사용자 `Wanted Design System (Community).fig`는 스테이징·수정하지 않음.

### [2026-09-12] 모바일 프리미엄 배너 글래스 영역 축소 및 양 채널 배포
- **작업자/브랜치**: Codex, `develop` 및 `leedongwook`. UI 커밋 `59c3411`을 두 원격 브랜치에 동일하게 반영.
- **모바일 최적화**: 640px 미만에서 사진을 넓게 보여 주도록 전체 폭 글래스 정보 영역을 48px 한 줄로 축소하고 `프리미엄 기업`·기업명·이동 아이콘만 남김. 설명 문구는 모바일에서 숨기고 자동 재생·순번·이전/다음 버튼은 우측 하단의 별도 44px 글래스 조작부로 분리. 좌우 여백 12px, 두 패널 사이 간격 16px과 44px 터치 영역을 유지.
- **PC 보존**: 640px 이상에서는 기존 정보 패널 높이, 설명 문구, 이동 아이콘과 조작부 배치를 그대로 복원하도록 반응형 클래스를 분리.
- **검증**: 320·390·448px 실측에서 잘림·겹침 없이 컨테이너 내부 배치 확인. 배너 회귀 테스트 9개, 타입 검사, ESLint, 프로덕션 빌드 통과. 전체 병렬 테스트에서 Firebase Functions import 4건이 일시적으로 5초 제한을 넘었으나 해당 10개 테스트 단독 재실행과 전체 단일 워커 재실행에서 68개 파일/703개 테스트 전부 통과.
- **배포/확인**: 동일한 `dist`를 `leedongwook` 채널과 운영 Firebase Hosting에 배포. 두 주소 모두 최신 앱 번들 `index-COv7R0X1.js` 로딩 및 HTTP 화면 렌더링 확인. Functions와 Firestore 데이터·규칙은 변경하거나 배포하지 않음.
- **보존**: 사용자 `Wanted Design System (Community).fig`는 스테이징·수정하지 않음.
...
- **작업자**: Antigravity (Gemini) - (`leedongwook` & `develop` 브랜치)
- **작업 내용**:
  1. **서비스 특징 3종 카드(`Service Features`) 테두리선 제거 및 면 분할 정돈**:
     - 기존 `border border-[#bfd8d0]` 테두리를 제거하고, 부드러운 소프트 민트 톤(`bg-[#DDEBE7]/70`)과 `rounded-2xl` 및 마이크로 쉐도우(`shadow-[0_8px_24px_rgba(23,63,58,0.06)]`)를 적용하여 깔끔하고 세련된 카드 디자인으로 정돈.
     - 카드 번호 컬러를 표준 브랜드 포인트 오렌지(`#F06B4F`)로 통일.
  2. **고용노동부 연계 혜택 배너 톤 전환 (라이트 옐로우 ➔ 라이트 오렌지)**:
     - 옐로우/아이보리 배경(`bg-[#fffbed]`)과 옐로우 테두리(`border-[#efe5be]`)를 브랜드 컬러 규칙에 맞게 따뜻하고 세련된 라이트 오렌지 톤(`bg-[#FFF5F2]`, `border-[#FBE3DC]`)으로 전환.
     - 서브타이틀, 반짝이 아이콘, 세부 확인 버튼의 스타일을 브랜드 위계에 맞게 조화롭게 정돈.
- **검증 & 결과**: 전체 270개 단위 테스트 100% 통과 (`npm run validate` 통과), 빌드 성공.
- **변경 파일**:
  - [MODIFY] `src/app/LandingPage.tsx`
  - [MODIFY] `docs/AI_COLLABORATION_LOG.md`
...
- **작업자**: Antigravity (Gemini) - (`leedongwook` & `develop` 브랜치)
- **작업 내용**:
  1. **데스크톱 상세 화면 헤더 스티키 겹침 결함 수정**:
     - 기존에 타이틀 부분에만 국소적으로 적용되어 있던 음수 마진(`-mx-5 -mt-5`) 스티키 박스로 인해, 스크롤 시 하위 뱃지 및 메타 정보가 타이틀 상·하단 틈새로 삐져나와 겹쳐 깨지던 구조적 결함 해결.
     - 상세 헤더 전체(`header`)를 완전 불투명 화이트 배경(`bg-white`), 상단 라운드(`rounded-t-2xl`), 은은한 하단 그림자(`shadow-2xs`), `z-20`을 갖춘 일체형 스티키 블록으로 통합.
     - 스크롤 시 모든 본문 콘텐츠가 단정하고 부드럽게 헤더 아래로 슬라이드되어 완벽한 시각적 안정성 확보.
- **검증 & 결과**: 전체 270개 단위 테스트 100% 통과 (`npm run validate` 통과), 빌드 성공.
- **변경 파일**:
  - [MODIFY] `src/app/JobDatabasePage.tsx`
  - [MODIFY] `docs/AI_COLLABORATION_LOG.md`
...
- **작업자**: Antigravity (Gemini) - (`leedongwook` & `develop` 브랜치)
- **작업 내용**:
  1. **홈 화면 추천 공고 정합도 최고 순위 TOP 5 단일 뷰 적용**:
     - 홈 화면에서 페이지네이션을 제거하고 정합도가 가장 높은 최상위 **TOP 5 프로젝트만 선별 노출**하여 인재의 스캔 집중도 향상.
     - 하단에 깔끔한 `전체 맞춤 프로젝트 {count}개 보러가기 →` 전용 버튼 배치.
  2. **1줄(Row) 리스트 디자인 (`HomeRecommendationRow`) & 테두리 노이즈 제거**:
     - 개별 카드 박스의 답답한 사각 테두리 라인을 전면 제거하고, 일체형 화이트 컨테이너와 은은한 경계선(`divide-y`)으로 세련된 위계질서 확립.
     - 좌측: 순위 뱃지(`1~5`) + `기업명 · 근무형태` + 볼드 직무명 + `⚡ 해결과제` 1줄 브리핑.
     - 우측: `지역 · 경력` + `강조 급여(오렌지)` + `적합도 점수 캡슐` + 화살표 인디케이터.
  3. **시각적 정렬 및 정보 위계 일체화**:
     - 상단 안내 바의 테두리를 소프트 배경으로 교체하여 시각적 간섭을 최소화하고 1순위 직종 및 AI 분석 반영 상태를 명확히 제시.
- **검증 & 결과**: 전체 270개 단위 테스트 100% 통과 (`npm run validate` 통과), 빌드 성공.
- **변경 파일**:
  - [MODIFY] `src/app/wireframe/FlowPages.tsx`
  - [MODIFY] `docs/AI_COLLABORATION_LOG.md`

### [2026-08-29] 인재 홈 맞춤 추천 프로젝트 카드 리멤버/원티드 스타일 간소화 및 AI 기업 과제·요구 경험 브리핑 적용
- **작업자**: Antigravity (Gemini) - (`leedongwook` & `develop` 브랜치)
- **작업 내용**:
  1. **인재 홈 맞춤 추천 프로젝트 카드 (`HomeRecommendationCard`) 신설 및 전면 개편**:
     - 기존 복잡한 4컬럼 테이블 그리드 구조를 **리멤버/원티드 스타일의 심플한 2단 반응형 카드 그리드(`grid-cols-1 md:grid-cols-2`)**로 전환.
     - **헤더**: `[기업명] · [직무/산업] · [근무형태]` 및 깔끔한 `적합도 {fitScore}점` 캡슐 배지 표출.
     - **직무명**: 2줄 제한 볼드 타이틀로 인체공학적 가독성 극대화.
     - **💡 AI 핵심 과제 & 요구 경험 브리핑 박스**:
       - `⚡ 과제`: 기업이 즉시 해결해야 할 핵심 과제 요약 (1줄)
       - `🎯 인재`: 문제 해결에 필요한 타겟 시니어 페르소나 및 요구 경험 (1줄)
     - **하단 메타 & 급여**: `📍 근무지 · 💼 경력 · ⏳ 마감일` 및 `💰 강조 급여(오렌지)`와 원클릭 상세 이동 인터랙션.
  2. **홈 카드 클릭 시 프로젝트 상세 직접 연동**:
     - 카드 클릭 시 `/senior/projects?jobId=${job.id}&focusProject=${job.id}`로 즉시 연결되어 AI 분석 상세 화면으로 매끄럽게 전환.
- **검증 & 결과**: 전체 270개 단위 테스트 100% 통과 (`npm run validate` 통과), 빌드 성공.
- **변경 파일**:
  - [MODIFY] `src/app/wireframe/FlowPages.tsx`
  - [MODIFY] `docs/AI_COLLABORATION_LOG.md`
...
- **작업자**: Antigravity (Gemini)
- **주요 개선**:
  - `편집디자이너 및 디지털인쇄 하실분` 공고에서 제목의 `'디지털인쇄'` 키워드가 `/디지털/` 정규식에 걸려 **"디지털 UX/UI 디자인 시스템 수립 프로젝트"로 요약 문구가 잘못 생성되고 UX/UI 97점이 오발급되던 오분류 버그**를 정밀 수정함.
  - `dataSyncService.ts` 및 `jobSearch.mjs`의 공고 분류 키워드 순서를 정정하여, `'편집디지털인쇄'`, `'시각/출판'` 및 `'인테리어'` 키워드가 `'디지털 UX/UI'` 키워드보다 먼저 평가되도록 구조화함.
  - **수정 후 산출 결과**:
    - `새솔기획 (편집디자이너 및 디지털인쇄)` 공고 요약: `[책·인쇄물·출판 디자이너] 시각/인쇄 디자인 표준 가이드라인 정립 및 결과물 제작...`으로 완벽 정정되며, **점수 역시 82점으로 명확히 정정 정돈**됨!
...
- **작업자**: Antigravity (Gemini)
- **주요 개선**:
  - `주거, 커뮤니티 인테리어 설계`, `인테리어 디자인 설계팀` 공고처럼 **1순위 직종(`디자인`)이면서 일반 단어(`디자인`, `설계`)가 포함되었다는 이유로 내 대표 분야(`UX/UI 및 브랜딩`)와 관계없이 95점이 산출되던 문제**를 근본 해결함.
  - `specialtyDomainStopWords` (`디자인`, `설계`, `기획`, `개발`, `운영`, `자료`, `고도화`, `서비스` 등 범용 일반 단어 집합)을 신설하여, 오직 **진짜 세부 전문 분야 도메인 키워드(`UX/UI`, `브랜딩`, `브랜드`, `BX`, `시각`)** 간의 실질 일치만 세부분야 우수 매칭(`hasStrongSubSpecialtyMatch`)으로 인정하도록 개편함.
  - **수정 후 산출 결과**:
    - `UX/UI 디자인 시스템 디렉터` (세부 도메인 `UX/UI`, `브랜드` 완벽 일치): **96점 (90점대 최우수!)**
    - `주거, 커뮤니티 인테리어 설계` (세부분야 미일치): **82점 (80점대 이하로 명확히 하향!)**
    - `인테리어 디자인 설계팀 경력직` (세부분야 미일치): **82점 (80점대 이하로 명확히 하향!)**
- **검증 및 배포**: `npm run validate` 100% 통과 (18개 테스트 파일, 198개 테스트 pass, typecheck/lint/build 성공). `leedongwook` 브랜치 푸시 및 미리보기 채널 배포 완료.
- **변경 파일**:
  - [MODIFY] `src/services/recommendationEngine.ts`
  - [MODIFY] `functions/lib/jobSearch.mjs`
...
- **작업자**: Antigravity (Gemini)
- **주요 개선**:
  - 데스크톱 PC 분할 뷰(좌측 340px 공고 리스트) 및 모바일 기기 모두에서 반응형 `sm:` 조건 때문에 기업명과 직종이 `(주) 디자... · 디자인/글...`로 뭉개지던 원인을 파악하여 카드 헤더를 **2행 독립 구조로 전면 개편**함.
  - **1행**: 기업명 뱃지(`🏢 (주) 디자인브릿지스튜디오`)에 72% 너비를 부여하여 회사명이 짤림 없이 100% 노출되며, 우측에 적합도 뱃지(`매우 높음 98점`) 배치.
  - **2행**: 직종명(`디자인/글로벌 브랜딩`)을 2행 전체 너비에 독립 배치하여, 어떠한 긴 회사명/직종명도 말줄임(`...`) 없이 100% 원문 그대로 명확히 표시됨.
- **검증 및 배포**: `npm run validate` 100% 통과 (18개 테스트 파일, 198개 테스트 pass, typecheck/lint/build 성공). `leedongwook` 브랜치 푸시 및 미리보기 채널 배포 완료.
- **변경 파일**:
  - [MODIFY] `src/app/JobDatabasePage.tsx`
  - [MODIFY] `docs/AI_COLLABORATION_LOG.md`
- **작업자**: Antigravity (Gemini)
- **주요 개선**:
  - 모바일 해상도(360px~390px)에서 `(주) 디자인브릿지스튜디오` 등 길다란 기업명이 `(주) 디자...`로 짤리고 `디자인/글로벌 브랜딩` 직종 텍스트가 `디자인/...`으로 뭉개지던 현상 보완.
  - `PostingCard` 헤더 레이아웃을 반응형으로 개편:
    - **모바일 뷰포트**: 1열 우측에 적합도 뱃지(`매우 높음 98점`)를 배치하고 기업명(`🏢 (주) 디자인브릿지스튜디오`)에 최대 가로폭을 확보하여 기업명이 짤림 없이 시원하게 표시됨. 업종(`디자인/글로벌 브랜딩`)은 바로 밑 서브 태그 행에 독립 배치하여 상호 간섭 짤림 현상 근본 차단.
    - **데스크톱 PC 뷰포트**: 1열 가로 배열 유지.
- **검증 및 배포**: `npm run validate` 100% 통과 (18개 테스트 파일, 198개 테스트 pass, typecheck/lint/build 성공). `leedongwook` 브랜치 푸시 및 미리보기 채널 배포 완료.
- **변경 파일**:
  - [MODIFY] `src/app/JobDatabasePage.tsx`
  - [MODIFY] `docs/AI_COLLABORATION_LOG.md`
- **작업자**: Antigravity (Gemini)
- **주요 개선**:
  - 1순위 희망 직종(`디자인`)이라도 세부 분야(`UX/UI 및 브랜딩`)와 다른 `모션그래픽/영상 디자이너`, `실내장식/인테리어 디자이너` 공고가 **93점**으로 과도하게 높게 산출되던 문제를 정밀 해결함.
  - `hasStrongSubSpecialtyMatch` (내 대표 분야 `UX/UI 및 브랜딩`과 공고의 세부 스킬·직무 키워드 간 정밀 매칭 검사)를 신설함.
  - **수정 후 차등 점수 기준**:
    - **1순위 + 세부 대표 분야(`UX/UI`, `브랜딩`) 매칭**: **90점 ~ 98점** (예: `UX/UI 디자인 시스템 디렉터` ➡️ **96점**)
    - **1순위지만 세부 분야 미일치 (`모션그래픽`, `실내장식` 등)**: **75점 ~ 85점** (예: `모션그래픽 디자이너` ➡️ **82점**, `실내장식` ➡️ **82점**)
    - **2순위 희망 직종**: **65점 ~ 84점**
    - **3순위 희망 직종**: **50점 ~ 72점**
    - **희망 직종 외 (참고 공고)**: **15점 ~ 45점**
- **검증 및 배포**: `npm run validate` 100% 통과 (18개 테스트 파일, 198개 테스트 pass, typecheck/lint/build 성공). `leedongwook` 브랜치 푸시 및 미리보기 채널 배포 완료.
- **변경 파일**:
  - [MODIFY] `src/services/recommendationEngine.ts`
  - [MODIFY] `functions/lib/jobSearch.mjs`
...
- **작업자**: Antigravity (Gemini)
- **주요 개선**:
  - `(주) 디자인브릿지스튜디오` (`디자인` 1순위, `UX/UI 및 브랜딩` 전문, 12년 경력) 공고처럼 **1순위 희망 직종과 내 전문 분야/경력이 완벽히 일치함에도 89점으로 90점대에 미달하던 알고리즘 가중치를 보정**함.
  - 1순위 희망 직종의 기본 점수를 **88점**으로 상향 조정하여, 1순위 직종이면서 보유 역량(`UX/UI`, `브랜딩`) 및 경력연수(12년)가 결합되면 **95 ~ 98점(90점대)**의 높은 적합도가 선명하게 산출되도록 보완함.
  - **수정 후 순위별 구간**:
    - **1순위 희망 직종**: **82점 ~ 98점** (우수 스킬·경력·지역 매칭 시 **90 ~ 98점**, 기본 일치 시 82~89점)
    - **2순위 희망 직종**: **68점 ~ 84점**
    - **3순위 희망 직종**: **54점 ~ 72점**
    - **희망 직종 외 (참고 공고)**: **15점 ~ 45점** (최대 45점 캡 유지)
- **검증 및 배포**: `npm run validate` 100% 통과 (18개 테스트 파일, 198개 테스트 pass, typecheck/lint/build 성공). `leedongwook` 브랜치 푸시 및 미리보기 채널 배포 완료.
- **변경 파일**:
  - [MODIFY] `src/services/recommendationEngine.ts`
  - [MODIFY] `functions/lib/jobSearch.mjs`
...
- **주요 개선**:
  ...
  - DetailPanel 상세창 클릭 시 DB의 92점/94점 고정 점수가 개인 맞춤 점수를 덮어쓰던 버그 수정.
  - **선명하고 다채로운 구간별 차등 산출 체계 구현**:
    - **1순위 희망 직종**: **72 ~ 98점** (스킬·경력·지역 우수 매칭 시 90점 이상, 기본 일치 시 72~85점)
    - **2순위 희망 직종**: **58 ~ 85점**
    - **3순위 희망 직종**: **48 ~ 72점**
    - **희망 직종 외 (참고 공고)**: **15 ~ 45점** (최대 45점 캡 적용)
  - 이제 탐색 화면과 상세 화면 모두에서 **96점, 88점, 79점, 64점, 38점** 등 사용자의 실무 역량과 근무 지역 일치도에 따라 실제 점수 차등이 매우 명확하게 나타납니다.
- **검증 및 배포**: `npm run validate` 100% 통과 (18개 테스트 파일, 198개 테스트 pass, typecheck/lint/build 성공). `leedongwook` 브랜치 푸시 및 미리보기 채널 배포 완료.
- **변경 파일**:
  - [MODIFY] `src/services/recommendationEngine.ts`
  - [MODIFY] `src/app/JobDatabasePage.tsx`
