...
- **PC 보존**: 640px 이상 일반 화면에서는 기존 기업 설명, 데스크톱 이동 아이콘, 자동 롤링과 정지 버튼 및 조작부 위치를 그대로 유지.
- **회귀 방지/검증**: 모바일 하단 단일 패널, 우측 조작부 예약 폭, 모바일 정지 버튼 숨김, 380px 라벨 규칙, PC 복원과 자동·수동 순환을 테스트에 고정. 첫 병렬 전체 테스트에서 변경과 무관한 Functions import 3건이 5초 제한을 넘었으나 단독 재실행 10/10 통과. 이후 `npm run validate` 전체 통과: 타입 검사, ESLint, 68개 테스트 파일/705개 테스트, 프로덕션 빌드 성공.
- **배포/확인**: 동일한 62개 `dist` 파일을 `leedongwook` 채널과 정식 Firebase Hosting에 순서대로 배포. 두 주소 모두 HTTP 200 및 동일한 JS `assets/index-Bu2fcfA9.js`, CSS `assets/index-y-fWlKpK.css`를 제공. 정식 `assets/FlowPages-Dilq6bQj.js`에서 하단 단일 패널, `pr-[7.75rem]` 예약 폭, 우측 한 줄 조작부, 모바일 정지 버튼 숨김과 기존 좌측 상단 조작부 제거를 직접 확인.
