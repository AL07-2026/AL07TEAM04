...
### [2026-08-19] 홈 및 프로젝트 리스트 `해결 프로젝트` 요약문 정제 & `근무·보상 조건` 심플 포맷터 개편
- **작업자**: Antigravity (Gemini) - (`leedongwook` 브랜치)
- **주요 내용**:
  - **`해결 프로젝트` 문구 가독성 및 정확도 향상 (`formatCleanProblemStatement`)**:
    - 기업명이나 타이틀이 이중 중복 표시되거나(`(주) 디자인브릿지스튜디오의 기업 글로벌 브랜드...`), 불필요한 기계식 텍스트(`...과제 해결입니다.`, `...모집합니다. 채용`)가 붙던 현상 완전 차단.
    - 기업명 및 공고 제목의 단순 반복 없이 공고의 진짜 세부 역량(UX/UI, 모션그래픽, 공간 인테리어, 메커니즘 설계 등)에 맞는 명확하고 고객 친화적인 과제 요약문 생성.
    - `J케이아트 (모션그래픽 디자이너)` 공고가 `시각/인쇄`로 오분류되던 판정 정규식을 보정하여 `모션그래픽 연출 및 비주얼 미디어 콘텐츠 제작`으로 100% 정확하게 정정.
  - **`근무·보상 조건` 심플화 포맷터 신설**:
